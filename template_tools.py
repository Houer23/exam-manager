...
objective_question_count: "" # 客观题数：题号大于该数均为主观题；留空=按题号格式自动判定
# 题型配置（可动态增删题型，配置非空时以配置为准）：
# 题型名, 数量或题号列表；多个用分号分隔；不区分全半角。
# 数量式允许在前面若干行（按顺序从 1 起连续分配），列表式后不允许再出现数量式；
# 列表式用逗号/短横线（如 1,2,3-5；"5,"=题号5）。
# 范围完全包含 -> 小范围为子题型；部分重叠报错。
question_types:
  客观题: 20
  单选: 1-10
  多选: 11-20
  主观题: 21-25
# 二分配置：true（默认）且顶层题型 <=2 时按 objective_question_count 分客观/主观，
# 配置题型作为子题型；false 时完全按配置题型划分
binary_split: true
